...
function summarizeLabels(name, labels) {
  ...
  for (const x of endpoints) {
    ...
    rows.push({
      N: x,
      labels: Math.min(labels.length, j + 2),
      pairs: count,
      mean,
      variance,
      se,
...
<text x="${pad}" y="${height - 18}" fill="#94a3b8" font-family="Menlo, Consolas, monospace" font-size="12">running mean of adjacent normalized centered gap products, x-axis log-scaled, N=${N}</text>
</svg>`;
}
...
const output = {
  ...
  preregisteredConfirm: "stable flat negative line whose effect size beats five Cramer and composite controls",
  preregisteredBreak: "known consecutive-gap anti-correlation/local-residue layer, unstable effect, or control reproduction",
  N,
  endpoints,
  real,
  cramer,
...
const jsonPath = path.join(outDir, `gapac1mean-audit-${N}.json`);
const svgPath = path.join(outDir, `gapac1mean-audit-${N}.svg`);
const mdPath = path.join(outDir, `gapac1mean-audit-${N}.md`);
fs.writeFileSync(jsonPath, JSON.stringify(output, null, 2));
fs.writeFileSync(svgPath, svg([real, ...cramer.slice(0, 3), ...composite.slice(0, 2)]));

const line = (s) => {
  const last = s.rows.at(-1);
  return `| ${s.name} | ${last.pairs} | ${last.mean.toFixed(8)} | ${last.se.toFixed(8)} | ${last.z.toFixed(3)} |`;
};
const md = `# gapac1mean audit
...
| N | pairs | mean | se | z |
| ---: | ---: | ---: | ---: | ---: |
${real.rows.map((r) => `| ${r.N} | ${r.pairs} | ${r.mean.toFixed(8)} | ${r.se.toFixed(8)} | ${r.z.toFixed(3)} |`).join("\n")}
...
console.log(JSON.stringify({ ok: true, jsonPath, mdPath, svgPath, real: real.rows.at(-1) }, null, 2));
