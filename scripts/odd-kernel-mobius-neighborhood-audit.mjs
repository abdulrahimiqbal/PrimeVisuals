...
function scoreValue(n, mu) {
  if (n < 3 || n + 1 >= mu.length || n % 2 === 0) return 0;
  return (mu[oddPartValue(n - 1)] || 0) * (mu[oddPartValue(n + 1)] || 0);
}

function localScoreValue(n, primes) {
  if (n < 3 || n % 2 === 0) return 0;
  let out = 1;
  for (const p of primes) {
    const left = n - 1;
    const right = n + 1;
    if (left % (p * p) === 0 || right % (p * p) === 0) return 0;
    if (left % p === 0) out *= -1;
    if (right % p === 0) out *= -1;
  }
  return out;
...
function scoreLabels(name, labels, mu, valueFactory = null, endpointCounts = null) {
  const sorted = labels.filter((n) => n >= 3 && n <= maxN && n % 2 === 1).sort((a, b) => a - b);
  ...
  const pushEndpoint = (N, targetCount = null) => {
    ...
    endpointRows.push({
      ...
      zero: count - nonzero,
      sum,
      normalized: count ? sum / Math.sqrt(count) : 0,
      nonzeroNormalized: nonzero ? sum / Math.sqrt(nonzero) : 0,
      maxAbs,
      maxAbsNormalized: count ? maxAbs / Math.sqrt(count) : 0,
      maxAbsNonzeroNormalized: nonzero ? maxAbs / Math.sqrt(nonzero) : 0,
    ...
    blockRows.push({
      ...
      sum: blockSum,
      normalized: blockCount ? blockSum / Math.sqrt(blockCount) : 0,
      nonzeroNormalized: blockNonzero ? blockSum / Math.sqrt(blockNonzero) : 0,
...
function matchedCompositeValues(labels, mu, pools, fallback, modulus, seed) {
  const rnd = mulberry32(seed);
  return labels.map((n) => {
    const bucket = pools[n % modulus];
    const source = bucket.length ? bucket : fallback;
    return scoreValue(source[Math.floor(rnd() * source.length)], mu);
  });
}
...
function functionFieldShells(q, maxDegree) {
  ...
  for (let degree = 1; degree <= maxDegree; degree++) {
    ...
    rows.push({
      ...
      primeNonzero,
      primeSum,
      primeNormalized: primeCount ? primeSum / Math.sqrt(primeCount) : 0,
      primeNonzeroNormalized: primeNonzero ? primeSum / Math.sqrt(primeNonzero) : 0,
      primeMean: primeCount ? primeSum / primeCount : 0,
      primeNonzeroRate: primeCount ? primeNonzero / primeCount : 0,
      monicCount,
      monicNonzero,
      monicSum,
      monicNormalized: monicCount ? monicSum / Math.sqrt(monicCount) : 0,
      monicNonzeroNormalized: monicNonzero ? monicSum / Math.sqrt(monicNonzero) : 0,
...
function makeSvg(real, families) {
  ...
  lines.push(`<text x="${margin.left}" y="38" fill="#e5e7eb" font-size="24" font-weight="700">Odd-kernel shifted Mobius neighborhood bridge</text>`);
  lines.push(`<text x="${margin.left}" y="66" fill="#94a3b8" font-size="15">X(n)=mu(oddpart(n-1))mu(oddpart(n+1)); y=max |sum X| / sqrt(label count)</text>`);
  for (let g = 0; g <= 4; g++) {
  ...
  for (const family of families) {
    for (const run of family.runs) {
      ...
      lines.push(`<path d="${d}" fill="none" stroke="${colors[family.key]}" stroke-width="1.5" opacity="0.36"/>`);
    }
  }
  ...
  lines.push(`<text x="${margin.left}" y="${H - 18}" fill="#cbd5e1" font-size="14">Endpoint real sum=${end.sum}, nonzero=${end.nonzero}, normalized=${fmt(end.normalized, 3)}, max/sqrt=${fmt(end.maxAbsNormalized, 3)}, theta=${fmt(fitTheta(real.endpointRows), 3)}</text>`);
  lines.push(`</svg>`);
  return lines.join("\n");
}
...
function markdownReport(data) {
  ...
  lines.push("", "## Small-prime local-only diagnostics", "");
  lines.push("| local primes | endpoint sum | endpoint normalized | endpoint max/sqrt | theta | corr(full, local) |");
  lines.push("| --- | ---: | ---: | ---: | ---: | ---: |");
  for (const diag of data.localDiagnostics) {
    const end = diag.run.endpointRows.at(-1);
    lines.push(`| ${diag.primes.join("*")} | ${end.sum} | ${fmt(end.normalized)} | ${fmt(end.maxAbsNormalized)} | ${fmt(fitTheta(diag.run.endpointRows))} | ${fmt(diag.correlation)} |`);
  }
  ...
  lines.push(`Endpoint local-composite max/sqrt range: \`${fmt(data.controlSummaries.localComposite[last].maxAbsNormalized[0])}..${fmt(data.controlSummaries.localComposite[last].maxAbsNormalized[1])}\`.`);
  lines.push("", "## Factor check", "");
  lines.push("The forced 2-adic zero from Cycle 66 is removed. The remaining danger is local small-prime structure: for primes p>3, one of p-1 or p+1 is divisible by 3; for p>5, residues modulo 5 decide whether a neighbor gets a first-power factor or a squarefactor zero; and so on. The local-composite control matches each prime to an odd composite with the same residue modulo 3^2*5^2*7^2, so reproduction there is evidence for a local-factor collapse rather than prime-specific regularity.");
  lines.push("", "## Files", "");
...
const localCompositeRuns = seeds.map((seed) => {
  const values = matchedCompositeValues(realLabels, mu, compositeResiduePools, compositeResidueFallback, localMod, seed);
  let index = 0;
  return scoreLabels(`local-composite-${seed}`, realLabels, mu, () => values[index++] ?? 0, endpointCounts);
});
const localDiagnostics = [
  ...
  const values = realLabels.map((n) => localScoreValue(n, ps));
  let index = 0;
  const run = scoreLabels(`local-${ps.join("-")}`, realLabels, mu, () => values[index++] ?? 0, endpointCounts);
  return { primes: ps, run, correlation: correlation(realValues, values) };
});
...
const baseName = `odd-kernel-mobius-neighborhood-${maxN}`;
...
console.log(JSON.stringify({
  ...
  localDiagnostics: localDiagnostics.map((diag) => ({
    primes: diag.primes,
    endpoint: diag.run.endpointRows.at(-1),
    theta: fitTheta(diag.run.endpointRows),
