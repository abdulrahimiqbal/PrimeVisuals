...
function buildOddStartBaselines(widths) {
  console.error(`[rough-offset] background for ${widths.length} widths with ${backgroundPerWidth} samples each`);
  const random = rng(0x57ab1e);
  const stats = new Map();
  for (const width of widths) {
    let sum = 0, sumSq = 0, exceptions = 0;
    const maxStart = Math.max(5, eventLimit - width - 1);
    for (let i = 0; i < backgroundPerWidth; i++) {
      let start = 3 + 2 * Math.floor(random() * Math.max(1, Math.floor((maxStart - 3) / 2)));
      if (start + width > eventLimit) start = Math.max(3, eventLimit - width - 1) | 1;
      const row = roughFirstFeature(start, width);
      sum += row.feature;
      sumSq += row.feature * row.feature;
      exceptions += row.exception;
    }
    const mu = sum / backgroundPerWidth;
    let sd = Math.sqrt(Math.max(0, sumSq / backgroundPerWidth - mu * mu));
    if (sd < 1e-9) sd = 1;
    stats.set(width, {
      width,
      count: backgroundPerWidth,
      mean: mu,
      sd,
      exceptionRate: exceptions / backgroundPerWidth,
    });
  }
  return stats;
}
...
function scoreLabels(name, labels, limit, baselines) {
  ...
  let featureSum = 0, exceptionCount = 0, sumSq = 0, gaps = 0;
  for (let i = 0; i + 1 < labels.length; i++) {
    const start = labels[i];
    if (start < 3) continue;
  ...
  return {
    name,
    gaps,
    meanFeature: featureSum / Math.max(1, gaps),
    exceptionRate: exceptionCount / Math.max(1, gaps),
    rmsZ: Math.sqrt(sumSq / Math.max(1, gaps)),
    ...bridge,
    topWidths,
  };
}
...
function auditInteger() {
  console.error(`[rough-offset] labels to ${eventLimit}`);
  ...
  const baselines = buildOddStartBaselines(widths);
  const rows = [];
  for (const limit of endpoints) {
    console.error(`[rough-offset] scoring N=${limit}`);
...
function markdownReport(result) {
  ...
  return `# gap-conditioned rough-witness offset bridge audit
...
\`${result.integer.widths.length}\`; max gap width: \`${result.integer.maxGap}\`.
...
function svgPlot(result) {
  ...
  const terminalValues = [
    { label: "real", value: rows.at(-1).real.terminalZ, color: "#67e8f9" },
    { label: "Cramer min", value: rows.at(-1).summary.cramer.terminalZ[0], color: "#facc15" },
    { label: "Cramer max", value: rows.at(-1).summary.cramer.terminalZ[1], color: "#facc15" },
    { label: "W2310 min", value: rows.at(-1).summary.wheel2310.terminalZ[0], color: "#34d399" },
    { label: "W2310 max", value: rows.at(-1).summary.wheel2310.terminalZ[1], color: "#34d399" },
    { label: "comp min", value: rows.at(-1).summary.composite210.terminalZ[0], color: "#fb7185" },
    { label: "comp max", value: rows.at(-1).summary.composite210.terminalZ[1], color: "#fb7185" },
  ];
  const maxTerminal = Math.max(...terminalValues.map((row) => Math.abs(row.value)), 1);
  const barX = 850, zeroX = barX + 130;
  const bars = terminalValues.map((row, i) => {
    const yy = 104 + i * 34;
    const w = 95 * Math.abs(row.value) / maxTerminal;
    const x0 = row.value >= 0 ? zeroX : zeroX - w;
    return `<text x="${barX}" y="${yy + 12}" fill="#cbd5e1" font-size="12">${row.label}</text>
<rect x="${x0}" y="${yy}" width="${w}" height="16" fill="${row.color}" opacity="0.86"/>
<text x="${zeroX + 105}" y="${yy + 12}" fill="#94a3b8" font-size="12">${fmt(row.value)}</text>`;
  }).join("\n");
...
<text x="70" y="32" fill="#e5e7eb" font-size="18" font-weight="700">Gap-conditioned rough-witness offset bridge</text>
<text x="70" y="55" fill="#a8b3c7" font-size="13">first rough witness offset / gap width, z-scored by same-width random odd starts</text>
...
<text x="850" y="74" fill="#e5e7eb" font-size="15" font-weight="700">Endpoint terminal z</text>
<line x1="${zeroX}" x2="${zeroX}" y1="90" y2="350" stroke="#334155"/>
${bars}
...
const basename = `rough-witness-offset-bridge-audit-${N}`;
const jsonPath = path.join(outDir, `${basename}.json`);
const mdPath = path.join(outDir, `${basename}.md`);
const svgPath = path.join(outDir, `${basename}.svg`);

const result = {
  candidate: "gap-conditioned rough-witness offset bridge",
