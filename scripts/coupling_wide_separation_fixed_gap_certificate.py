#!/usr/bin/env python3
"""Exact fixed-gap entrance clock from the wide-separation central band.
...
subclocks.  The x-coordinate target interval is chosen so that |z-y|<99/100 for
every y in the source enclosure; the y-coordinate interval is analogous.
Add an inward x -> x+log(q) or y -> y-log(q) atom for q=2,3,4,5 whenever the
whole source cell sends that atom into both |m|<4/5 and r<99/100.  The arch
target intervals obey the same two target inequalities.  Every selected event
therefore enters this common compact lower set with a fixed separation gap,
and every arch density is a cellwise constant lower envelope dominated
uniformly over its source interval.
...
r<99/100 to r>1 in a later global phase ledger.

Every selected target has |midpoint|<4/5 and separation <99/100.  Reflection
and exchange give all symmetry copies.

Reproduce with:

    PYTHONPATH=/tmp/pvdeps python3 \
      scripts/coupling_wide_separation_fixed_gap_certificate.py
"""
...
# The n>=5 theta tail is globally bounded on t>=0 by the same decreasing
# Gaussian majorant used in the local coupling certificates.
...
def K_even(t: arb) -> arb:
    ...
    # The signed evaluation contains the positive-branch range on [0,radius].
...
def integrate_one_marginal(left: arb, right: arb,
    ...
    for index in range(ARCH_CELLS):
        ...
        kval = K_even(z).lower()
        density = kval * source_density_lower(z, source_box)
        if density > 0:
            total += density * width
    return total
...
def arch_target(other_box: tuple[arb, arb]) -> tuple[arb, arb]:
    # Both z-other and (z+other)/2 obey their strict target inequalities
    # throughout the source box.
    left = max(
        -TARGET_COORD,
        other_box[1] - TARGET_R + TARGET_GAP,
        -2 * TARGET_M - other_box[0] + TARGET_GAP,
    )
    right = min(
        TARGET_COORD,
        other_box[0] + TARGET_R - TARGET_GAP,
...
def selected_prime_powers(m_left: arb, m_right: arb,
                          r_left: arb, r_right: arb,
                          direction: int) -> tuple[int, ...]:
    selected = []
    for prime_power in (2, 3, 4, 5):
        logq = LOGS[prime_power]
        target_m_left = m_left + direction * logq / 2
        target_m_right = m_right + direction * logq / 2
        if (
            max(abs(r_left - logq), abs(r_right - logq)) < TARGET_R
            and target_m_left > -TARGET_M
            and target_m_right < TARGET_M
        ):
...
def certify_cell(m_left: arb, m_right: arb,
    ...
    selected_qx = selected_prime_powers(
        m_left, m_right, r_left, r_right, +1
    )
    selected_qy = selected_prime_powers(
        m_left, m_right, r_left, r_right, -1
    )

    arch = integrate_one_marginal(zx[0], zx[1], x_box)
    arch += integrate_one_marginal(zy[0], zy[1], y_box)
    prime = minimum_prime_rate(x_box, +1, selected_qx)
    prime += minimum_prime_rate(y_box, -1, selected_qy)
    selected = arch + prime

    assert separation_bound(zx, y_box) < TARGET_R
    assert separation_bound(x_box, zy) < TARGET_R
    assert midpoint_bound(zx, y_box) < TARGET_M
    assert midpoint_bound(x_box, zy) < TARGET_M
    for prime_power in selected_qx:
        logq = LOGS[prime_power]
        assert max(abs(r_left - logq), abs(r_right - logq)) < TARGET_R
        assert m_left + logq / 2 > -TARGET_M
        assert m_right + logq / 2 < TARGET_M
    for prime_power in selected_qy:
        logq = LOGS[prime_power]
        assert max(abs(r_left - logq), abs(r_right - logq)) < TARGET_R
        assert m_left - logq / 2 > -TARGET_M
        assert m_right - logq / 2 < TARGET_M
...
def main() -> None:
    ...
    def certify_or_split(ml: arb, mr: arb, rl: arb, rr: arb,
                         depth: int = 0) -> None:
        nonlocal worst, worst_cell, leaf_count, maximum_depth
        rate = certify_cell(ml, mr, rl, rr)
        if not rate > q(1, 2):
            assert depth < 4, (ml, mr, rl, rr, rate)
            mm = (ml + mr) / 2
            rm = (rl + rr) / 2
        ...
        if worst is None or rate.lower() < worst.lower():
            ...
            worst_cell = (
                ml, mr, rl, rr,
                selected_prime_powers(ml, mr, rl, rr, +1),
                selected_prime_powers(ml, mr, rl, rr, -1),
            )
    ...
    print("adaptive_leaf_count:", leaf_count)
    print("maximum_refinement_depth:", maximum_depth)
    print("worst_cell_(mL,mR,rL,rR,qx,qy):", worst_cell)
    print("worst_selected_rate:", worst)
    print("worst_margin_over_half:", worst - q(1, 2))
    print("target: |midpoint|<4/5 and separation<99/100")
